...
static ThresholdState ntc_state;
static ThresholdState dht_state;
...
void setup() {
    ...
    conditioning_init(&ntc_state, DEFAULT_THRESHOLD,
                      DEFAULT_HYSTERESIS, DEBOUNCE_TIME_MS);
    conditioning_init(&dht_state, DEFAULT_THRESHOLD,
                      DEFAULT_HYSTERESIS, DEBOUNCE_TIME_MS);
}
...
void loop() {
    ...
    /*Fast acquisition loop (NTC + potentiometer)*/
    if (now - last_sensor_ms >= SENSOR_INTERVAL_MS) {
        ...
        /* Conditioning – NTC */
        conditioning_update(&ntc_state, ntc_temp, now);
        alert_set(LED_NTC_PIN, conditioning_is_alert(&ntc_state));
    }

    /*Slower DHT22 read (min 2 s interval)*/
    if (now - last_dht_ms >= DHT_INTERVAL_MS) {
        ...
        /* Conditioning – DHT */
        conditioning_update(&dht_state, dht_temp, now);
        alert_set(LED_DHT_PIN, conditioning_is_alert(&dht_state));
    }

    /*Display / reporting (500 ms)*/
    if (now - last_display_ms >= DISPLAY_INTERVAL_MS) {
        last_display_ms = now;

        display_update(ntc_temp, ntc_raw, conditioning_is_alert(&ntc_state),
                       dht_temp, dht_humid, conditioning_is_alert(&dht_state),
                       threshold);
    }
}